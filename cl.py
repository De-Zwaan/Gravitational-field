...
def BWcompute(width, height, planets):
    ...
    print("Computing...")
    prg.BWcompute(queue, res.shape, None, np.int32(width), np.int32(height), planet_x_buf, planet_y_buf, planet_mass_buf, dest_buf)
    print("Done.")
...
def colorCompute(width, height, planets):
    planet_x =          np.empty(len(planets)).astype(np.float32)
    planet_y =          np.empty_like(planet_x)
    planet_mass =       np.empty_like(planet_x)
    ...
    # Run the program using the buffers
    print("Computing...")
    prg.colorCompute(queue, res.shape, None, np.int32(width), np.int32(height), planet_x_buf, planet_y_buf, planet_mass_buf, planet_color_buf, dest_buf)
    print("Done.")
